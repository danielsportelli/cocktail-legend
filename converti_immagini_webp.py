"""
CONVERTI IMMAGINI JPG / JPEG / PNG → WebP
==========================================
Istruzioni:
...
def converti():
    ...
    files = [f for f in os.listdir(CARTELLA) if os.path.splitext(f)[1].lower() in ESTENSIONI]

    if not files:
        print("❌ Nessun file JPG, JPEG o PNG trovato nella cartella 'immagini/'.")
        return

    print(f"▶ Trovati {len(files)} file da convertire. Inizio conversione...\n")
    ...
    for filename in sorted(files):
        percorso_originale = os.path.join(CARTELLA, filename)
        nome_senza_ext = os.path.splitext(filename)[0]
        percorso_webp = os.path.join(CARTELLA, nome_senza_ext + ".webp")

        try:
            dim_prima = os.path.getsize(percorso_originale)
            peso_prima += dim_prima

            with Image.open(percorso_originale) as img:
    ...
    print("   I file originali sono ancora lì — cancellali quando sei sicuro che tutto funziona.")
